...
void ParticleSystem::setHeight(int minHeight, int maxHeight) {
	minHeight = minHeight;
	maxHeight = maxHeight;
}
...
//
ImpulseRadialForce::ImpulseRadialForce(float magnitude, int min, int max) {
...
void ImpulseRadialForce::updateForce(Particle * particle) {
	...
	//
	ofVec3f dir = ofVec3f(ofRandom(-1, 1), ofRandom(min, max), ofRandom(-1, 1));
